...
export default function App() {
  ...
  // Boot splash: handwriting intro → app reveal
  useEffect(() => {
    const t = setTimeout(() => setBooting(false), 2600);
    return () => clearTimeout(t);
  }, []);

  // Typewriter tagline — the Terminal theme's only entrance animation
  useEffect(() => {
    if (booting) return;
    let i = 0;
    const t = setInterval(() => {
  ...
  const handleFilesSelected = useCallback((files) => {
    ...
    for (const f of files) {
      if (!ALLOWED.test(f.name) || !f.type.startsWith('image/')) {
    ...
    if (valid.length > 0) {
      setQueue((prev) => [...prev, ...valid].slice(0, 30));
    }
  }, []);
        ...
        <>
          ...
          <header className="js-intro mb-4 border-b border-line pb-4">
            <p className="term-caret text-sm text-ink-mid sm:text-base">
              <span className="text-ink">pico</span> --local --no-cloud --private
            </p>
            <p className="mt-2 min-h-[1.6em] text-xs text-ink-dim sm:text-sm" aria-live="polite">
              {typed}
            </p>
          </header>
          ...
          <main className="space-y-6">
              ...
              {queue.length > 0 && (
                <div className="term-panel flex flex-wrap items-center gap-2 p-3">
                  ...
                  <div className="flex min-w-0 flex-1 flex-wrap gap-1.5 overflow-hidden">
                    ...
                    {queue.length > 6 && (
                      <span className="bg-surf-muted px-2 py-1 text-[length:var(--text-md)] text-ink-dim">
                        +{queue.length - 6} more
                      </span>
                    )}
                  </div>
            ...
            <button
              ...
              className={`term-btn relative w-full ${busy ? 'pico-shimmer overflow-hidden' : ''}`}
              aria-busy={busy}
            >
              {busy
            ...
            {status === 'compressing' && (
              <div className="flex justify-center">
                <LoadingState label="running sharp" variant="Drive" />
              </div>
            )}

            {error && (
              <p role="alert" className="border border-err/60 bg-surf-muted px-4 py-3 text-xs leading-relaxed text-err">
                ✕ {error}
              </p>
            )}
