...
export default function LoadingState({
  ...
  return (
    <div className="flex w-fit items-center gap-2.5">
      <span
        aria-hidden
        className="pico-loader-grid grid grid-cols-[repeat(3,4px)] gap-[1.5px]"
      >
        {delays.map((d, i) => (
          <span
            key={i}
            className={`size-[4px] bg-[var(--foreground)] ${round ? "rounded-full" : "rounded-[1px]"}`}
            style={{
      ...
      <span
        className="pico-loader-label bg-clip-text text-[13px] font-medium text-transparent"
        style={{
          backgroundImage:
      ...
      <span className="font-mono text-[12px] text-[var(--muted-foreground)] tabular-nums">
        {elapsed}
      </span>
    </div>
  );
